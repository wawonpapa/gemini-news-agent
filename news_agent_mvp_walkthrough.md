...
Inoreaderなどの有料サービスやAPI接続の設定は一切不要になり、非常にシンプルなステップで導入可能です。
...
4. 実行する関数に `initSpreadsheet` を選択し、**「実行」** ボタンを押します（初回のみGoogleへのアクセス承認プロンプトが出るので、許可してください）。
...
3. 次に、**[MonthlyDigest.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/MonthlyDigest.gs)** を開き、関数選択で `monthlyDigestJob` を選択し、**「実行」** をクリックします。
