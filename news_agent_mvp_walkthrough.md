...
6. **[Actions.gs](news_agent_mvp/Actions.gs)**:
   - GAS Web Appの `doGet(e)` および `doPost(e)` 処理を担当。メール内のアクションリンク確認画面の表示のほか、自分で見つけたニュース記事の登録フォームの表示および、送信されたURLのスクレイピング・AI自動解析・DB保存・興味プロファイル学習の処理フローを担当します。
7. **[WeeklyDigest.gs](news_agent_mvp/WeeklyDigest.gs)**:
   - **【マスタードキュメント追記型】**週次のNotebookLM向けインプット生成を担当。毎週新しいファイルを量産するのではなく、Googleドライブ上の単一のドキュメント `📖 Master News Agent Archive` の末尾に最新記事を自動追記（アペンド）します。
...
### 4. 🔠 メールやWeb App内の絵文字や記号が文字化け（?や）する場合
*   **原因**: HTMLメール、テキストメール、またはGASのWeb App画面上にUnicodeの4バイト絵文字（💡, 📝, 🚀, 🎉）や特殊記号（↗）を生の文字として直接記述していると、受信メーラー側でのデコードエラーや、clasp同期時・GASデプロイ時の文字コード誤認識によって文字化けが発生します。
*   **解決方法 (HTMLメール & Web App UI)**:
    *   [Notify.gs](news_agent_mvp/Notify.gs) や [Actions.gs](news_agent_mvp/Actions.gs) などのHTMLを出力するソースコード内では、生の絵文字を直接書かず、必ず **「HTML数値エンティティコード（例: `&#128161;` [💡], `&#128221;` [📝], `&#128640;` [🚀], `&#127881;` [🎉], `&#8599;` [↗]）」** に置き換えて記述することで、文字コード環境に依存せず100%文字化けを防止できます。
*   **解決方法 (テキストメール - 更新通知)**:
    *   実体参照が使えないプレーンテキストメールにおいては、絵文字を一切排除し、**`[週次レポート]` や `■` などの安全な標準の全角テキスト記号に置き換えて送信**します。
