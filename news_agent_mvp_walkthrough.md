...
3. 次に、**[MonthlyDigest.gs](news_agent_mvp/MonthlyDigest.gs)** を開き、関数選択で `monthlyDigestJob` を選択し、**「実行」** をクリックします。
...
*   **解決方法 (テキストメール - 月次更新通知)**:
    *   実体参照が使えないテキストメールにおいては、絵文字を一切排除し、**`[月次レポート]` や `■` などの安全な標準の全角テキスト記号に置き換えて送信**します。

### 5. 📂 月次ダイジェスト（monthlyDigestJob）で「DocumentApp.create 権限不足」エラーが出る場合
