...
| **#14** | 類似ニュースの重複配信防止（同一トピック・同イベントに関する記事のフィルタリング） | Gemini またはタイトル類似度による意味的重複チェックを実装し、同一イベントを扱う異なる記事が複数配信されないようにする。 | [Code.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/Code.gs), [SearchAgent.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/SearchAgent.gs) |
| **#13** | 配信されるニュース件数を毎日10件以上に維持するためのロジック改善 | クエリ数の増加・temperature調整・ソースドメインの拡張などで、重複フィルタ通過後も毎日10件以上の高品質記事が配信されるようにする。 | [Code.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/Code.gs), [SearchAgent.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/SearchAgent.gs) |
...
| デプロイメントID | ターゲット | 役割 |
| :--- | :--- | :--- |
| `<ACTIVE_DEPLOYMENT_ID>` | `@23` | **本番（アクティブ）** — 全通知メールに埋め込まれているURLはこちら。必ずここに再デプロイすること。`clasp deployments` で確認。 |
