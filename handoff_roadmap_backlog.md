...
| **#34** | 配信ニュースに数週間前の古いニュース記事が混入する問題の改善 | Google Search Grounding または GAS側で `published_at`（公開日）を判定し、直近（過去24〜48時間以内）のニュース以外をフィルタリングする。 | [SearchAgent.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/SearchAgent.gs), [Code.gs](file:///d:/Git/News_Agent_Test/news_agent_mvp/Code.gs) |
