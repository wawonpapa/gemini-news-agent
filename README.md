...
flowchart TD
    ...
    subgraph SUBMIT_PROCESS["⚙️ 手動登録の解析処理"]
        P["🤖 Gemini API\n対象記事のスクレイピング & 構造化解析\n(要約・重要度・カテゴリ等の自動抽出)"]
    end
...
### 5. 自分で見つけたニュースの手動登録 (Web UI)
デイリーメールのフッターに常設された登録ボタン（WebアプリURL）から、ユーザーが自身で見つけた任意のニュース記事URLとコメントを入力できるWebフォームを提供します。送信すると、AIが即座に対象Webページをスクレイピング・解析（要約、カテゴリ、タグ、重要度の抽出）してスプレッドシート（DB）に `status = 'good'` で保存し、興味プロファイルを自動で学習更新します。
...
├── Actions.gs             # Webアプリ doGet(e)/doPost(e) 処理（メールアクション受付、カスタムニュース登録フォーム表示・解析保存処理）
...
Gemini APIは無料プランでも稼働可能ですが、より安定した運用と制限のない快適なAI検索を行うために、Google AI Studioの「従量課金プラン（Pay-as-you-go）」に移行することをお勧めします。
本システムでは、コスト最適化のために超高効率モデルである **`gemini-3.1-flash-lite`** を採用しています。

*   **Google検索グラウンディング利用料**: 1検索クエリにつき `$0.014`（約 2.2 円 ※$1=155円換算）
*   **APIトークン使用料（Gemini 3.1 Flash-Lite）**:
    *   入力: `$0.25 / 100万トークン`
    *   出力: `$1.50 / 100万トークン`
    *   ※検索グラウンディング実行時は、検索結果のWebコンテンツ自動読み込みにより1回あたりの入力トークン数が1万〜1.5万トークンに増加します（トークン代: 1検索あたり約 0.8 円）。

#### 毎朝1回（3クエリ検索）稼働させた場合のコスト試算
*   **1日あたり**: **約 12 円 / 日** （1回あたり約 4.1 円 × 3 ＋ 基本料金）
*   **1ヶ月（30日）あたり**: **約 370 円 / 月**
*   ※GASのスクリプトプロパティで `LITE_MODE = true`（2クエリ検索）に設定した場合、さらに **約 8 円 / 日（約 250 円 / 月）** に抑えられます。
